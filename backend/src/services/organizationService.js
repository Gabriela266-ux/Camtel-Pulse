    ...
    async getFrontendHierarchy(centerId) {
        if (!centerId) {
            ...
            return {
                ...
                da: centres.flatMap((centre) => (centre.das || []).map((da) => ({
                    id: da.id, nom: da.nom, code: da.code, region: da.region, numero_sim: da.numero_sim,
                    code_zone: da.code_zone, nom_reseau: da.nom_reseau,
                    dsm: (da.dsms || []).map((dsm) => ({
                        id: dsm.id, nom: dsm.nom, numero_telephone: dsm.numero_telephone,
                        code_dsm: dsm.code_dsm, code_zone: dsm.code_zone, nom_reseau: dsm.nom_reseau,
                        pos: (dsm.pos_list || []).map((p) => ({
                            id: p.id, nom: p.nom, numero_telephone: p.numero_telephone,
                            code_pos: p.code_pos, code_dsm: p.code_dsm, code_zone: p.code_zone, nom_reseau: p.nom_reseau,
                        })),
                    })),
                }))),
            };
